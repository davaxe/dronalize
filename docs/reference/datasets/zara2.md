...
| Benchmark family    | ETH/UCY                             | Shares setup conventions with `eth`, `hotel`, `univ`, and `zara1`.  |
...
Dronalize does not currently rely on a dataset-specific version marker for ZARA2. The raw layout used by the loader does not expose a stable release version.
