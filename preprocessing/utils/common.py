# Copyright 2024-2025, Theodor Westny. All rights reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from __future__ import annotations

import os
import shutil
import sys
from fractions import Fraction
from typing import TYPE_CHECKING, Any

import numpy as np
import torch
from pandas import DataFrame
from scipy.signal import decimate, resample_poly
from sklearn.model_selection import train_test_split

MIN_SCORE_LEN = 3  # Minimum target length in seconds for multi-agent scoring

if TYPE_CHECKING:
    from argparse import Namespace

    from numpy.typing import NDArray
    from pandas import Series


def create_tensor_dict(
    input_array: NDArray[np.floating],
    target_array: NDArray[np.floating],
    agent_ids: list[int],
    agent_types: list[int],
    sample_freq: int,
    downsample_factor: int = 1,
    filter_order: int = 1,
    *,
    additional_features: list[str] | None = None,
    intentions: list[int] | None = None,
    determine_scored_ids: bool = False,
    k_max: int = 8,
    categories: NDArray[np.integer] | None = None,
) -> dict[str, Any] | None:
    """Create a dictionary of tensors from the input and target arrays."""
    if downsample_factor > 1:
        input_array = decimate_nan_enhanced(
            input_array,
            pad_order="front",
            ds_factor=downsample_factor,
            fz=sample_freq,
            filter_order=filter_order,
        )
        target_array = decimate_nan_enhanced(
            target_array,
            pad_order="back",
            ds_factor=downsample_factor,
            fz=sample_freq,
            filter_order=filter_order,
        )

    # Convert to tensors
    input_tensor = torch.from_numpy(input_array).float()
    target_tensor = torch.from_numpy(target_array).float()

    if determine_scored_ids:
        fr = torch.tensor([x[..., 0].unique().size() for x in input_tensor])
        to = torch.tensor([y[..., 0].unique().size() for y in target_tensor])

        non_scored_ids = torch.where(fr + to <= MIN_SCORE_LEN)[0]

        # Skip iteration if the TA is in non_scored_ids
        if 0 in non_scored_ids:
            return None
    else:
        non_scored_ids = None

    if categories is not None:
        categories = torch.from_numpy(categories).long()

    three_sec = 3 * sample_freq / downsample_factor

    input_mask, valid_mask, sa_mask, ma_mask = get_masks(
        input_tensor,
        target_tensor,
        int(three_sec),
        non_scored_ids,
        k_max,
        categories,
    )

    # Handle NaN values
    input_tensor[torch.isnan(input_tensor)] = 0.0
    target_tensor[torch.isnan(target_tensor)] = 0.0

    data = {
        "num_nodes": len(agent_ids),
        "ta_index": 0,
        "ids": agent_ids,
        "type": torch.tensor(agent_types).long(),
        "inp_pos": input_tensor[..., :2],
        "inp_vel": input_tensor[..., 2:4],
        "inp_acc": input_tensor[..., 4:6],
        "inp_yaw": input_tensor[..., 6:7],
        "trg_pos": target_tensor[..., :2],
        "trg_vel": target_tensor[..., 2:4],
        "trg_acc": target_tensor[..., 4:6],
        "trg_yaw": target_tensor[..., 6:],
        "input_mask": input_mask,
        "valid_mask": valid_mask,
        "sa_mask": sa_mask,
        "ma_mask": ma_mask,
    }

    if intentions is not None:
        data["intention"] = torch.tensor(intentions).long()

    if additional_features is not None:
        data.update({
            "inp_r1": input_tensor[..., 7:8],
            "inp_r2": input_tensor[..., 8:],
        })

    return data


def erase_previous_line(*, double_jump: bool = False) -> None:
    """Erase the previous line in the terminal."""
    sys.stdout.write("\x1b[1A")  # Move the cursor up one line
    sys.stdout.write("\x1b[2K")  # Clear the entire line
    if double_jump:
        sys.stdout.write("\x1b[1A")


def create_directories(args: Namespace, dataset: str | None = None) -> str:
    """Create directories for processed data."""
    if dataset is None:
        data_dir = args.dataset + args.add_name
    else:
        data_dir = dataset + args.add_name

    output_dir = os.path.join(args.output_dir, data_dir)

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    elif args.debug:
        return output_dir
    else:
        print(f"Folder {output_dir} already exists.")
        inp = input("Would you like to overwrite it? (y/n) \n").lower()
        if inp == "y":
            print("Overwriting folder...")
            # Clear folder
            try:
                shutil.rmtree(output_dir)
            except FileNotFoundError as e:
                print(f"Failed to delete {output_dir}. Reason: {e}")
            else:
                os.makedirs(output_dir)
        else:
            print("Exiting...")
            sys.exit()

    # create subdirectories
    if not os.path.exists(output_dir + "/train"):
        os.makedirs(output_dir + "/train")
    if not os.path.exists(output_dir + "/val"):
        os.makedirs(output_dir + "/val")
    if not os.path.exists(output_dir + "/test"):
        os.makedirs(output_dir + "/test")

    return output_dir


def compute_velocity(tracks: DataFrame, dt: float) -> DataFrame:
    """Estimate the velocity for each track in the DataFrame."""
    out = tracks.copy()

    def calc_velocity_component(series: Series) -> float | NDArray[np.float64]:
        """Calculate velocity for a single coordinate series."""
        if len(series) == 1:
            return 0.0
        return np.gradient(series.to_numpy(), dt, edge_order=1)

    out["vx"] = out.groupby("track_id", sort=False)["x"].transform(
        calc_velocity_component,
    )
    out["vy"] = out.groupby("track_id", sort=False)["y"].transform(
        calc_velocity_component,
    )

    return out


def compute_acceleration(tracks: DataFrame, dt: float) -> DataFrame:
    """Estimate the acceleration for each track in the DataFrame."""
    out = tracks.copy()

    def calc_acceleration_component(series: Series) -> float | NDArray[np.float64]:
        """Calculate acceleration for a single coordinate series."""
        if len(series) == 1:
            return 0.0
        return np.gradient(series.to_numpy(), dt, edge_order=1)

    out["ax"] = out.groupby("track_id", sort=False)["vx"].transform(
        calc_acceleration_component,
    )
    out["ay"] = out.groupby("track_id", sort=False)["vy"].transform(
        calc_acceleration_component,
    )

    return out


def get_frame_split(
    n_frames: int,
    seed: int = 42,
    test_size: float = 0.2,
) -> tuple[NDArray[np.integer], NDArray[np.integer], NDArray[np.integer]]:
    """Split the frames into train, validation, and test sets."""
    all_frames = list(range(1, n_frames + 1))

    # Divide all frames into ten lists of equal length
    frame_lists = np.array_split(all_frames, 10)

    # Split the lists into train (80%), validation (10%), and test (10%) sets
    train, valtest = train_test_split(
        frame_lists,
        test_size=test_size,
        random_state=seed,
    )
    val, test = train_test_split(valtest, test_size=0.5, random_state=seed)

    # Sort the order of the arrays in the list and concatenate them
    train = np.concatenate(sorted(train, key=lambda x: x[0]))
    val = np.concatenate(sorted(val, key=lambda x: x[0]))
    test = np.concatenate(sorted(test, key=lambda x: x[0]))
    return train, val, test


def update_frames(
    agent_frames: NDArray[np.integer],
    alt_set1: NDArray[np.integer],
    alt_set2: NDArray[np.integer],
) -> NDArray[np.integer]:
    """Remove frames from the agent_frames that are in alt_set1 or alt_set2.

    :param agent_frames: array of frames that are in the agent set
    :param alt_set1: array of frames that are in an alternative set
    :param alt_set2: array of frames that are in another alternative set
    :return:
    """
    if agent_frames[-1] <= agent_frames[0]:
        msg = "The frames are not in the correct order"
        raise ValueError(msg)
    idx_b = np.isin(agent_frames, alt_set1)
    idx_c = np.isin(agent_frames, alt_set2)
    idx = np.logical_or(idx_b, idx_c)
    return agent_frames[~idx]


def class_list_to_int_list(class_list: list[str]) -> list[int]:
    """Convert a list of class names to a list of integers.

    :param class_list:
    :return:
    """
    class_to_int = {
        "car": 0,
        "van": 0,
        "trailer": 0,
        "truck": 1,
        "truck_bus": 2,
        "bus": 2,
        "motorcycle": 3,
        "bicycle": 4,
        "pedestrian": 5,
        "tricycle": 6,
        "animal": 7,
        "undefined": 8,
    }
    return [class_to_int[c] for c in class_list]


def get_other_sets(current_set: str) -> list[str] | None:
    """Get the other sets (train, val, test) based on the current set.

    :param current_set: The current set
    :return: The other sets
    """
    match current_set:
        case "train":
            return ["val", "test"]
        case "val":
            return ["train", "test"]
        case "test":
            return ["train", "val"]
        case _:
            return None


def get_neighbors(
    df: DataFrame,
    frame: int,
    id0: int,
    driving_dir: int | None = None,
) -> DataFrame:
    """Get the vehicles (except id0) present at a given frame."""
    if driving_dir is None:
        df1 = df[(df.frame == frame) & (df.track_id != id0)]
    else:
        df1 = df[
            (df.frame == frame)
            & (df.track_id != id0)
            & (df.drivingDirection == driving_dir)
        ]
    return df1


def get_meta_property(
    tracks_meta: DataFrame,
    agent_ids: list,
    prop: str = "class",
) -> list[str]:
    """Get a meta property of the agent from the tracks_meta DataFrame."""
    return [
        tracks_meta[tracks_meta.track_id == v_id][prop].to_numpy()[0]
        for v_id in agent_ids
    ]


def get_maneuver(
    tracks: DataFrame,
    frame: int,
    agent_ids: list,
    prop: str = "maneuver",
) -> list[int]:
    """Get the maneuver of the agents at a given frame."""
    return [
        tracks[(tracks.track_id == v_id) & (tracks.frame == frame)][prop].to_numpy()[
            0
        ]
        for v_id in agent_ids
    ]


def get_features(
    df: DataFrame,
    frame_start: int,
    frame_end: int,
    n_features: int,
    track_id: int,
    add_features: str | list[str] | None = None,
) -> NDArray[np.floating]:
    """Get the features of the agent with id track_id in the frame range [frame_start, frame_end]."""
    feature_list = ["x", "y", "vx", "vy", "ax", "ay", "psi"]

    if add_features is not None:
        if isinstance(add_features, str):
            feature_list.append(add_features)
        else:
            feature_list.extend(add_features)

    if n_features != len(feature_list):
        msg = "The number of features should match the length of the feature list"
        raise ValueError(msg)

    return_array = np.empty((frame_end - frame_start + 1, n_features))
    return_array[:] = np.nan

    if track_id != -1:
        dfx = df[
            (df.frame >= frame_start)
            & (df.frame <= frame_end)
            & (df.track_id == track_id)
        ]
    else:
        dfx = df[(df.frame >= frame_start) & (df.frame <= frame_end)]
    try:
        first_frame = dfx.frame.to_numpy()[0].astype(int)
    except IndexError:
        return return_array
    frame_offset = first_frame - frame_start

    features = dfx[feature_list].to_numpy()

    return_array[frame_offset : frame_offset + features.shape[0], :] = features

    return return_array


def decimate_nan_enhanced(
    x: NDArray[np.floating],
    pad_order: str = "front",
    ds_factor: float = 5.0,
    fz: int = 25,
    max_s: float = 1.0,
    filter_order: int = 7,
) -> NDArray[np.floating]:
    """Enhanced version that handles both integer and rational downsampling factors.

    For rational factors (like 2.5), it uses resample_poly for proper anti-aliasing.
    For integer factors, it uses the original decimation approach.

    Args:
        x: Input array of shape (n_agents, n_timesteps, n_features)
        pad_order: "front" or "back" - where to pad with NaNs
        ds_factor: Downsampling factor (can be integer or float)
        fz: Original sampling frequency
        max_s: Maximum seconds threshold for decimation
        filter_order: Filter order for integer decimation

    Returns:
        Downsampled array with NaN padding

    """
    is_integer_factor = float(ds_factor).is_integer()
    decimation_bound = max_s * fz

    if is_integer_factor:
        up, down = 1, int(ds_factor)
        target_ds_len = int(x.shape[1] / ds_factor)
    else:
        frac = Fraction(ds_factor).limit_denominator(100)
        up, down = frac.denominator, frac.numerator
        target_ds_len = int(x.shape[1] * up / down)

    y = np.full((x.shape[0], target_ds_len, x.shape[2]), np.nan)
    not_nan_idx = ~np.isnan(x[..., 0])
    decimation_check = not_nan_idx.sum(axis=1) >= decimation_bound
    decimation_idx = np.where(decimation_check)[0]
    lazy_idx = np.where(~decimation_check)[0]

    for idx in decimation_idx:
        arr = x[idx]

        # slice out the non-nan values
        arr = arr[not_nan_idx[idx]][::-1]

        if is_integer_factor:
            arr = decimate(arr, down, n=filter_order, axis=0, zero_phase=True)[::-1]
        elif arr.shape[0] < 2 * max(up, down):
            indices = np.linspace(arr.shape[0] - 1, 0, num=target_ds_len, dtype=int)
            arr = arr[indices][::-1]
        else:
            arr = resample_poly(
                arr,
                up,
                down,
                axis=0,
                window=("kaiser", 5.0),
                padtype="smooth",
            )[::-1]

        # Ensure the array is not longer than the target length
        arr = arr[:target_ds_len]

        # pad the array with NaNs to the target length along the first axis
        if pad_order == "front":
            n_pad = ((target_ds_len - arr.shape[0], 0), (0, 0))
            arr = np.pad(arr, n_pad, mode="constant", constant_values=np.nan)
        elif pad_order == "back":
            n_pad = ((0, target_ds_len - arr.shape[0]), (0, 0))
            arr = np.pad(arr, n_pad, mode="constant", constant_values=np.nan)
        else:
            msg = "pad_order should be either 'front' or 'back'"
            raise ValueError(msg)

        y[idx] = arr

    for idx in lazy_idx:
        if is_integer_factor:
            indices = np.arange(x.shape[1] - 1, -1, -down)
        else:
            indices = np.linspace(x.shape[1] - 1, 0, num=target_ds_len, dtype=int)

        indices = indices[:target_ds_len]  # Ensure we don't exceed target length

        sampled_data = x[idx][indices][::-1]

        y[idx, : sampled_data.shape[0]] = sampled_data

    return y


def decimate_nan(
    x: NDArray[np.floating],
    pad_order: str = "front",
    ds_factor: int = 5,
    fz: int = 25,
    max_s: float = 1.0,
    filter_order: int = 7,
) -> NDArray[np.floating]:
    decimation_bound = max_s * fz
    target_ds_len = int(x.shape[1] / ds_factor)
    y = np.zeros((x.shape[0], target_ds_len, x.shape[2]))

    not_nan_idx = ~np.isnan(x[..., 0])
    decimation_check = not_nan_idx.sum(axis=1) >= decimation_bound
    decimation_idx = np.where(decimation_check)[0]
    lazy_idx = np.where(~decimation_check)[0]

    for idx in decimation_idx:
        arr = x[idx]

        # slice out the non-nan values
        arr = arr[not_nan_idx[idx]][::-1]

        # decimate the array
        arr = decimate(arr, ds_factor, n=filter_order, axis=0, zero_phase=True)[::-1]

        # pad the array with NaNs to the target length along the first axis
        if pad_order == "front":
            n_pad = ((target_ds_len - arr.shape[0], 0), (0, 0))
            arr = np.pad(arr, n_pad, mode="constant", constant_values=np.nan)
        elif pad_order == "back":
            n_pad = ((0, target_ds_len - arr.shape[0]), (0, 0))
            arr = np.pad(arr, n_pad, mode="constant", constant_values=np.nan)
        else:
            msg = "pad_order should be either 'front' or 'back'"
            raise ValueError(msg)

        y[idx] = arr

    for idx in lazy_idx:
        y[idx] = x[idx, -1:0:-ds_factor][::-1]

    return y


def get_masks(
    x: torch.Tensor,
    y: torch.Tensor,
    ma_frames: int = 15,
    non_scored_ids: torch.Tensor | None = None,
    k_max: int = 8,
    categories: torch.Tensor | None = None,
    ta_index: int = 0,
) -> list[torch.Tensor]:
    """Get masks for the input and output tensors.

    There are four different masks that are created:

    1. input_mask: Mask for the input tensor (contains True for valid values (not NaNs))

    2. valid_mask: Mask for the output tensor (contains True for valid values (not NaNs)).
    This mask is recommended to be used for the loss calculation during training.

    3. ta_mask: Mask for the target agent (contains True for the target agent).
    This mask is used to identify the target agent in the single-target task.
    It is recommended to be used for quantitative metrics calculation for validation and testing.

    4. ma_mask: Mask for the multi-agent task (contains True for the surrounding agents).
    This mask is used to identify the surrounding agents in the multi-target task.
    It is recommended to be used for quantitative metrics calculation for validation and testing.
    For the multi-target task, the 8 closest agents to the target agent are selected.
    The requirement is that the agent should be visible for at least 3 seconds in to the future.

    :param x: The input tensor
    :param y: The output tensor
    :param ma_frames: The number of frames for the multi-agent mask (should represent 3 seconds)
    :param non_scored_ids: The ids of the agents that should not be scored
    :param k_max: The maximum number of agents to consider for the multi-agent mask
    :param ta_index: The index of the target agent in the input tensor
    :return: input_mask, valid_mask, tv_mask, mv_mask
    """
    input_mask = torch.isnan(x).sum(dim=2) == 0
    valid_mask = torch.isnan(y).sum(dim=2) == 0

    # Target agent mask. This will always have adequate number of samples by design
    ta_mask = torch.zeros_like(valid_mask)
    ta_mask[ta_index] = True

    # Multi-agent mask
    if categories is not None:
        # Score according to category (AV2 has 4 categories: 0: Fragment, 1:
        # Unscored, 2: Scored, 3: Focal)
        ma_mask = torch.zeros_like(valid_mask)
        ma_mask[categories > 1] = True

    else:
        # Score according to the number of frames

        # Only interested in indices where valid mask is True for at least 3 s
        long_idx = torch.where(valid_mask.sum(dim=1) >= ma_frames)[0]

        # Remove non-scored agents from long_idx (e.g., parked vehicles)
        if non_scored_ids is not None:
            long_idx = long_idx[~torch.isin(long_idx, non_scored_ids)]

        # (find the (max) 8 closest agents)
        dist = torch.norm(x[long_idx, -1, :2] - x[0, -1, :2], dim=1)
        max_k = min(k_max, len(long_idx))  # topk fails if k > input_tensor.shape[0]
        _, indices = torch.topk(dist, max_k, largest=False)

        intersection = long_idx[indices]

        ma_mask = torch.zeros_like(valid_mask)
        ma_mask[intersection] = True

    ma_mask = ma_mask & valid_mask  # only keep indices where valid mask is True

    return [input_mask, valid_mask, ta_mask, ma_mask]


def dummy_map() -> dict:
    """Create a dummy map with no nodes and edges.

    Returns:
        dict: A dictionary representing a dummy map with no nodes and edges.

    """
    # Add dummy map node type and edges
    map_point = {
        "num_nodes": 0,
        "type": torch.zeros((0,), dtype=torch.long),
        "position": torch.zeros((0, 2), dtype=torch.float),
    }
    edge_index_dummy = torch.zeros((2, 0), dtype=torch.long)
    edge_type_dummy = torch.zeros((0, 1), dtype=torch.long)

    return {
        "map_point": map_point,
        ("map_point", "to", "map_point"): {
            "edge_index": edge_index_dummy,
            "type": edge_type_dummy,
        },
    }
