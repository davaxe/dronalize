...
def find_target_vehicle(data, case_id):
    ...
    """
    track_ids = data[data["case_id"] == case_id]["track_id"].unique()
    max_duration = 0
    ego_id = None

    for track_id in track_ids:
        track = data[(data["case_id"] == case_id) & (data["track_id"] == track_id)]
...
def classify_ped_bike_simple(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[ped_bike_mask, "agent_type"] = df.loc[
        ped_bike_mask, ["case_id", "track_id"]
    ].apply(
        lambda row: type_map.get(
            (row["case_id"], row["track_id"]), "pedestrian/bicycle"
        ),
        axis=1,
    )
...
def classify_ped_bike_robust(df):
    ...
    df["angular_velocity"] = (df["vx"] * df["ay"] - df["vy"] * df["ax"]) / (
        df["speed"] + 1e-6
    )
...
def compute_trajectory_derivatives(tracks, dt: float = 0.1):
    ...
    results = (
        tracks.groupby(["case_id", "track_id"])[["vx", "vy"]]
