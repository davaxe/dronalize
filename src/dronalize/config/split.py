...
class SplitWeights(BaseModel):
    ...
    def active_weights(self) -> tuple[float, ...]:
        ...
        return tuple(w for w in self.values() if w > 0)
...
class SplitConfig(BaseModel):
    ...
    def resolve_runtime_input(
        ...
        *,
        ...
        supported_split_methods: Sequence[SplitStrategyName] = (),
        ...
        if strat_name == "auto" or (strat_name is None and split_weights is not None):
            strat_name = _resolve_auto_strategy(
                dataset_name, supported_split_methods, recommended_split_method
            )
        ...
        if strategy.type not in supported_split_methods:
            raise SplitMethodNotSupportedError(
                dataset_name, strategy.type, tuple(supported_split_methods)
            )
...
def _resolve_strategy(
    ...
    split_gap: int | None = None,
    split_n_segments: int | None = None,
    norm_splits: Sequence[DatasetSplit] = (),
    previous_strategy: SplitStrategy | None = None,
) -> SplitStrategy:
    split_vals = {s.value for s in norm_splits}
    strategy_map: dict[str | None, SplitStrategy | None] = {
    ...
    return strategy_map.get(strategy_name) or previous_strategy or Unsplit()


def _resolve_auto_strategy(
    dataset_name: str,
    supported_methods: Sequence[SplitStrategyName],
    recommended_method: SplitStrategyName | None,
) -> SplitStrategyName:
    ...
    msg = "Specify a split method explicitly."
    raise ConfigurationError(msg)
