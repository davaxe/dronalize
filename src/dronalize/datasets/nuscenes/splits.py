...
TRAIN_SCENES: Final[set[str]] = set(train_detect + train_track)

VAL_SCENES: Final[set[str]] = \
...
def get_split(name: str) -> DatasetSplit:
    """Get the dataset split for a given scene name."""
    if name in TRAIN_SCENES:
        return DatasetSplit.TRAIN
    if name in VAL_SCENES:
        return DatasetSplit.VAL
    msg = f"Scene {name} is not in any split."
