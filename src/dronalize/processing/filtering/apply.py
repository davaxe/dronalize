...
from dronalize.processing.filtering.agent import invalid_agent_tolerance_expr
from dronalize.processing.filtering.base import AgentCheckRule, rule_name
...
if TYPE_CHECKING:
    ...
    from dronalize.processing.filtering.base import CleanupRule, Rule, SceneCheckRule
...
def _diagnose_scene(
    ...
    *,
    ...
    diagnosed, diagnostic_columns, scene_pass_columns = _annotate_scene_rules(
        cleaned, scene_filter.scene_rules, ctx
    )
    return _annotate_agent_rules(
        diagnosed,
        scene_filter.agent_rules,
        ctx,
...
def _apply_cleanup(
    data: DataFrameT, rules: tuple[CleanupRule, ...], ctx: FilterContext
) -> DataFrameT:
    return data.filter(_and_all([rule.expr(ctx) for rule in rules]))


def _annotate_scene_rules(
    data: DataFrameT, rules: tuple[SceneCheckRule, ...], ctx: FilterContext
...
def _apply_scene_rule(
    data: DataFrameT, rule: SceneCheckRule, ctx: FilterContext
...
def _apply_agent_rule(
    data: DataFrameT, rule: AgentCheckRule, ctx: FilterContext
) -> tuple[DataFrameT, RuleDiagnostics]:
    columns = RuleDiagnostics.from_rule(rule)
