...
from dronalize.processing.filtering.base import RuleId, SceneCheckRule
from dronalize.processing.filtering.context import (
    AgentSelector,
    FrameInput,
    FrameSet,
    coerce_frame_set,
...
class AgentRange(SceneCheckRule):
    ...
    selector: AgentSelector | None = None
    type: Literal["agent_range"] = Field("agent_range", repr=False, init=False)
...
class CategoryRange(SceneCheckRule):
    ...
    type: Literal["category_range"] = Field("category_range", repr=False, init=False)

    @classmethod
    def define(cls, *ranges: tuple[str, Range]) -> CategoryRange:
        """Alternate constructor that accepts one or many category-range pairs."""
        return cls(ranges=dict(ranges))
...
SceneCheckSpec = Annotated[
    MinimumAgents | AgentRange | CategoryRange | RequireFrames | RequireWindow | MaxMissingFrames,
