"""Tolerance models for agent-level filtering rules.

## Import guide

```python
from dronalize.processing.filtering import Tolerance, tol
from dronalize.processing.filtering import AbsoluteTolerance, RelativeTolerance
```

Use these models when an agent rule should allow some invalid agents without
failing the whole scene immediately.

- [`AbsoluteTolerance`][dronalize.processing.filtering.AbsoluteTolerance] allows
  a fixed number of invalid agents
- [`RelativeTolerance`][dronalize.processing.filtering.RelativeTolerance]
  allows a fraction of invalid agents
- [`CombinedTolerance`][dronalize.processing.filtering.CombinedTolerance]
  requires both thresholds to be satisfied

[`tol`][dronalize.processing.filtering.tol] is the compact constructor for
programmatic rule definitions.

## Related modules

- [`dronalize.processing.filtering.agent`][] for agent rules that consume
  tolerance
- [`dronalize.processing.filtering.base`][dronalize.processing.filtering.base]
...
Tolerance = Annotated[
    RelativeTolerance | AbsoluteTolerance | CombinedTolerance,
    Field(discriminator="kind"),
]
"""Discriminated union for filter tolerance configuration."""
...
def tol(*, relative: float | None = None, absolute: int | None = None) -> Tolerance:
    ...
    This is a convinience function for tolerance. It infers the appropriate
