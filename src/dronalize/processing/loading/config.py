"""Configuration models for loader-side preprocessing and source loading."""
...
from dronalize.processing.filtering.filter import Filter  # noqa: TC001
...
class LoaderConfigUpdate(TypedDict, total=False):
    ...
    filter: Filter | None
    lane_change_sampling: LaneChangeSamplingConfig | None
...
class LaneChangeSamplingConfig(BaseModel):
...
class LoaderConfig(BaseModel):
    ...
    lane_change_sampling: LaneChangeSamplingConfig | None = Field(default=None)
    ...
    def with_lane_change_sampling(
        ...
        """Return a copy with lane-change sampling enabled."""
        return self._updated(
            lane_change_sampling=LaneChangeSamplingConfig(
                persist=persist,
