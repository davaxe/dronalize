...
def test_exclude_categories_removes_matching_rows() -> None:
    ...
    scene_filter = Filter.define_cleanup(
        cleanup.ExcludeCategories.define(categories=[AgentCategory.UNIMPORTANT])
    )
    ...
    assert_frame_equal(
        result,
        df.filter(pl.col("category") != AgentCategory.UNIMPORTANT),
    )
...
def test_include_categories_keeps_only_requested_rows() -> None:
    ...
    scene_filter = Filter.define_cleanup(
        cleanup.IncludeCategories.define(categories=[AgentCategory.CAR, AgentCategory.PEDESTRIAN])
    )
    ...
    assert_frame_equal(
        result,
        df.filter(pl.col("category").is_in([AgentCategory.CAR, AgentCategory.PEDESTRIAN])),
    )
...
def test_prune_by_rule_removes_all_rows_for_failing_agents() -> None:
    ...
    scene_filter = Filter.define_cleanup(
        cleanup.PruneByRule(rule=agent.MaxMissingFrames(maximum=0))
    )
...
def test_prune_by_rule_evaluates_agents_within_each_scene() -> None:
    ...
    scene_filter = Filter.define_cleanup(
        cleanup.PruneByRule(rule=agent.RequireFrames.define(frames=[0, 1]))
    )
