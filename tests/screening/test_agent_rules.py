...
from dronalize.processing.screening import AgentSelector, Screen, agent, scene, screen_scene, tol
...
def test_agent_frames_drop_scene() -> None:
    ...
    scene_screening = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)],
...
def test_tolerance_allows_one_invalid() -> None:
    ...
    tolerant_result = screen_scene(df, tolerant, group_by="scene")
...
def test_max_gap() -> None:
    ...
    strict = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)], agent_rules=[agent.MaxGap(maximum=0)]
    )
    tolerant = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)], agent_rules=[agent.MaxGap(maximum=1)]
    )
...
def test_agent_window_uses_tolerance() -> None:
    ...
    assert len(screen_scene(df, strict, group_by="scene", category_column="category")) == 0
    assert_frame_equal(screen_scene(df, tolerant, group_by="scene", category_column="category"), df)
...
def test_min_samples_drops_scene() -> None:
    ...
    scene_screening = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)], agent_rules=[agent.MinSamples(minimum=3)]
    )
    result = screen_scene(df, scene_screening, group_by="scene")
...
def test_starts_by_frame() -> None:
    ...
    strict = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)], agent_rules=[agent.StartsByFrame(frame=0)]
    )
    tolerant = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)], agent_rules=[agent.StartsByFrame(frame=1)]
    )
...
def test_ends_after_frame() -> None:
    ...
    strict = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)], agent_rules=[agent.EndsAfterFrame(frame=2)]
    )
    tolerant = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)], agent_rules=[agent.EndsAfterFrame(frame=1)]
    )
...
def test_min_span_uses_extent() -> None:
    ...
    passing = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)], agent_rules=[agent.MinSpan(minimum=3)]
    )
    failing = Screen.define(
        scene_rules=[scene.AgentRange(minimum=0)], agent_rules=[agent.MinSpan(minimum=4)]
    )
