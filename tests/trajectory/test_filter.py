...
def test_no_scene_filtering() -> None:
    ...
    result = filter_scene(df, *config.filter_args(), group_by="scene")
    assert len(result) == 2
...
def test_filter_agent_category() -> None:
    ...
    result = filter_scene(df, *config.filter_args(), group_by="scene", category_column="category")
...
def test_require_all_valid() -> None:
    ...
    result_strict = filter_scene(df, *config_strict.filter_args(), group_by="scene")
    ...
    result_lenient = filter_scene(df, *config_lenient.filter_args(), group_by="scene")
...
def test_no_groupby() -> None:
    ...
    result = filter_scene(df, *config.filter_args(), group_by=None)
