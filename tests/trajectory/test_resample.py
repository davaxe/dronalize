...
from dronalize.ops.trajectory.resample import Resampling, resample
...
def test_no_resampling() -> None:
    ...
    result = resample(df, Resampling(1, 1))
...
def test_downsample_by_two() -> None:
    ...
    result = resample(df, Resampling(1, 2))
...
def test_upsample_by_two() -> None:
    ...
    result = resample(df, Resampling(2, 1))
...
def test_fraction_simplification() -> None:
    ...
    result = resample(df, Resampling(2, 2))
    assert_frame_equal(result, df)
...
def test_group_by_isolation() -> None:
    ...
    result = resample(df, Resampling(1, 2), group_by="id")
...
def test_add_first_derivative() -> None:
    ...
    result = resample(df, Resampling(1, 1), add_derivative=True, dt=1.0)
...
def test_add_second_derivative() -> None:
    ...
    result = resample(df, Resampling(1, 1), add_second_derivative=True, dt=1.0)
...
def test_derivative_rename() -> None:
    ...
    rename = {1: ["vx", "vy"]}
    result = resample(df, Resampling(1, 1), add_derivative=True, dt=1.0, derivative_rename=rename)
...
def test_forward_fill_column() -> None:
    ...
    result = resample(df, Resampling(2, 1), group_by="id", forward_fill=["agent_category"])
...
def test_lazyframe_input() -> None:
    ...
    result = resample(lf, Resampling(1, 1))
...
def test_group_by_upsample_with_derivative() -> None:
    ...
    result = resample(df, Resampling(2, 1), group_by="id", add_derivative=True, dt=1.0)
    agent1 = result.filter(pl.col("id") == 1).sort("frame")
    agent2 = result.filter(pl.col("id") == 2).sort("frame")
...
def test_spline_method_basic() -> None:
    ...
    result = resample(df, Resampling(2, 1, "spline"), group_by="id")
...
def test_downsample_preserves_frame_column_name() -> None:
    ...
    result = resample(df, Resampling(1, 3))
