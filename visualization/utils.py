...
def plot_map_edges(
    ...
    for i in range(map_edge_index.shape[1]):
        ...
        elif map_edge_type[i] in (4, 5, 6, 7, 8, 9, 13):  # Lane markings
            plt.plot(
                map_pos[edge, 0],
                map_pos[edge, 1],
                color="darkgray",
...
def plot_agent_trajectories(
    ...
    for i in range(pos.shape[0]):
        ...
        ax.scatter(
            pos[i, -1, 0], pos[i, -1, 1], color=color, marker="*", s=100, alpha=1.0
        )
...
def visualize_batch(
    ...
    batch_idx: int = 0,
    *,
    use_ma_idx: bool = False,
    plot_supp: bool = False,
    save_fig: bool = False,
) -> None:
    ...
    sa_mask = data["agent"]["sa_mask"][batch]
    sa_idx = torch.where(sa_mask[:, 0])[0] if use_ma_idx else 0

    # Clean position data
    pos_eq_zero = pos == 0
    pos_eq_zero[sa_idx] = False
    ...
    # Process map edges
    map_edge_index, map_edge_type = subgraph(
        map_batch, map_edge_index, map_edge_type, relabel_nodes=True
    )
